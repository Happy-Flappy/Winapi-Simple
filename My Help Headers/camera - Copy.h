...
class Camera
{
    ...
    void ToScreen(double worldX,double worldY,double worldZ, int &screenX,int &screenY,float &ZScale)
    {
        ...
        double rotatedY = relativeY * cosPitch - relativeZ * sinPitch;
        double rotatedZ = relativeY * sinPitch + relativeZ * cosPitch;
        ...
        float scale = perspective / rotatedZ;
        
        screenX = (relativeX * scale) + (screenWidth/2);
        screenY = (rotatedY * scale) + (screenHeight/2);
